...
class Header:
    ...
    def parse_ipv4_hdr(self):
        ...
        self.ipv4.ver = self.header["ipv4"]["ver"]
        self.ipv4.ihl = self.header["ipv4"]["ihl"]
        self.ipv4.tos = self.header["ipv4"]["tos"]
        self.ipv4.total_length = self.header["ipv4"]["total_length"]
        self.ipv4.identification = self.header["ipv4"]["identification"]
        self.ipv4.flags = self.header["ipv4"]["flags"]
        self.ipv4.fragment_offset = self.header["ipv4"]["fragment_offset"]
        self.ipv4.ttl = self.header["ipv4"]["ttl"]
        self.ipv4.protocol = self.header["ipv4"]["protocol"]
        self.ipv4.check_sum = self.header["ipv4"]["check_sum"]
        self.ipv4.src_addr = self.header["ipv4"]["src_addr"]
        self.ipv4.dest_addr = self.header["ipv4"]["dest_addr"]
    ...
    def parse_udp_hdr(self):
        ...
        self.udp.src_port = self.header["udp"]["src_port"]
        self.udp.dest_port = self.header["udp"]["dest_port"]
        self.udp.length = self.header["udp"]["length"]
        self.udp.check_sum = self.header["udp"]["check_sum"]
